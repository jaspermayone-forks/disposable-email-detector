...
const loadDomains = async () => {
  ...
  const disposableDomainsBuffer = await fs.readFile(
...
export class ExternalSourceError extends Error {
  constructor(
    message: string = "URL must be provided when loading from external source"
  ) {
    super(message);
...
const loadDomainsFromExternalSource = async ({
  github,
  url,
}: LoadFromSource) => {
  if (cachedDomains) return cachedDomains;
  ...
  try {
    ...
    throw new ExternalSourceError(
      "Failed to load disposable domains from the provided URL"
    );
  }
};
...
export default async function disposableEmailDetector(
  email: string,
  options?: Options
): Promise<boolean> {
  try {
    ...
    if (options?.loadFromSource) {
      disposableDomains = await loadDomainsFromExternalSource(
        options.loadFromSource
      );
    } else {
    ...
    if (error.code === "ENOENT") {
      console.error(
        "index.json not found. Please create it with disposable domains."
      );
    } else if (error instanceof SyntaxError) {
      console.error(
        "Invalid JSON format in index.json. Please correct the file."
      );
